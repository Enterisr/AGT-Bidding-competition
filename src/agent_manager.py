...
class AgentManager:
    ...
    def load_agent(
        ...
        try:
            ...
            logger.error(
                f"Error registering agent for team {team_id}: {e}", exc_info=True
            )
            return None
    ...
    def execute_bid_with_timeout(
        self, agent: Any, item_id: str
        ...
        # FAST MODE: Skip multiprocessing for testing
        try:
            # Load and instantiate agent
            import importlib.util
            spec = importlib.util.spec_from_file_location(f"agent_{team_id}", metadata['file_path'])
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            ...
            # Execute bid
            bid = agent_instance.bidding_function(item_id)
            execution_time = time.time() - start_time
            
            # Save state
            new_state = {}
            for key, value in agent_instance.__dict__.items():
                if not key.startswith("_") and not callable(value):
                    new_state[key] = value
            ...
            execution_time = time.time() - start_time
            logger.error(f"Team {team_id}: Error in bid execution: {e}")
            return 0.0, execution_time, f"Error: {str(e)}"

    def update_agent_after_round(
        self, agent: Any, item_id: str, winning_team: str, price_paid: float
    ) -> bool:
        """
        ...
        try:
            ...
            # Update
            agent_instance.update_after_each_round(item_id, winning_team, price_paid)
            ...
            logger.error(f"Team {team_id}: Error in update: {e}")
            return False
